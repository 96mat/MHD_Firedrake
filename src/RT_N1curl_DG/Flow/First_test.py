...
dt=0.01; g=1e+1; eta=3e-6; R_h=5e-1; nu=1e-3; S=1; re_m=100; eps=1e-10
n=FacetNormal(mesh)
...
# BC
g_lid=as_vector((1.0,0))
g_wall=as_vector((0,0))
bc_v_L=DirichletBC(W.sub(0),u_in,1)
bc_v_L1=DirichletBC(W.sub(0),Constant((0,0)),(4,3))
bc_B_L=DirichletBC(W.sub(1),Constant((0,0)),(1,4,3))
bc_w = DirichletBC(W.sub(3), Constant(0.0), (1,2,3,4))
bc_J = DirichletBC(W.sub(4), Constant(0.0), (1,3,4))
bc_E = DirichletBC(W.sub(5), Constant(0.0), (1,4,3))
...
sigma = Constant(10) * W.sub(0).ufl_element().degree()**2
...
-inner(avg(nu*grad(u)),jump(outer(v, n)))*dS\
    ...
    + inner((sigma/avg(h))*avg(nu) * jump(outer(u, n)),jump(outer(v, n))) * dS\
- inner(outer(u , n), nu*grad(v)) * ds(4)\
    + inner((sigma/h) * nu * (u), v) * ds(4)   # penalità
- inner(outer( g_lid, n), nu*grad(v)) * ds(4)\
    + inner((sigma/h) * nu * (g_lid), v) * ds(4)   # penalità
