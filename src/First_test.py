...
#B_ic_onZ=ic_magnetic_regular(x[0],x[1])
# IC 2
...
dt=0.01; g=1e-2; eta=1e-6; R_h=1e-2
n=FacetNormal(mesh)
...
# %%
print('value',divB_check(Bh,mesh)) #check divB=0
# %%
# vtk output for Paraview
...
for ii in range(1, len(t_vec)):     # start from 1 to skip t=0
    ...
    print('div(B)_RT',norm(div(Bh),'L2'))
    ...
    #------
    print('div(B)_MSE',divB_check(Bh,mesh))
    print(f'Pe_Fluid= {CFL_Pe_eval(mesh,uh,dt,g)[0]}, CFL_Fluid= {CFL_Pe_eval(mesh,uh,dt,g)[1]}')
    print(f'Pe_Mag= {CFL_Pe_eval_mag(mesh,Bh,dt,eta)[0]}, CFL_Mag= {CFL_Pe_eval_mag(mesh,Bh,dt,eta)[1]}')
    #------
